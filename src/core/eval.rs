...
impl LExpr {
    #[inline]
    pub fn run_fn(&self, func: Function, args: &[LExpr], mem: Rc<RefCell<Memory>>) -> LitValue {
    ...
    #[inline]
    pub fn eval(&self, mem: Rc<RefCell<Memory>>) -> LitValue {
        match self {
            LExpr::Value { lit, .. } => lit.clone(),
            ...
            LExpr::Var { name, .. } => {
                ...
                if let Some((LitValue::HeapRef(addr), _)) = mem_ref.get_stack_var(name.lexeme) {
                    ...
                    println!(
                        "{}",
                        format!(
                            "warning: no variable '{}' was found on the heap",
                            name.lexeme
                        )
                        .yellow()
                        .bold()
                ...
                println!(
                    "{}",
                    format!(
                        "warning: no variable '{}' was found on the stack",
                        name.lexeme
                    )
                    .yellow()
                    .bold()
                        ...
                        if let Metadata::Var { is_const, .. } = metadata {
                            if is_const {
                                println!(
                                    "{}",
                                    format!(
                                        "warning: can't assign to a constant variable '{}'",
                                        name.lexeme
                                    )
                                    .yellow()
                                    .bold()
            ...
            LExpr::Null => LitValue::Nil,
        }
    }

    /// evaluate unary operation expression
    fn eval_unary(&self, op: &Token, rhs: &LitValue) -> LitValue {
        match (op.lexeme, rhs) {
            // `-a`, minus unary operation
            ("-", &LitValue::Int(a)) => LitValue::Int(-a),
            ("-", &LitValue::Flt(a)) => LitValue::Flt(-a),
            // TODO
            //
            _ => unimplemented!(),
        }
    ...
    fn eval_binary(&self, lhs: &LitValue, op: &Token, rhs: &LitValue) -> LitValue {
        match (lhs, op.lexeme, rhs) {
            // binary operations of `int`
            (LitValue::Int(a), "+", LitValue::Int(b)) => LitValue::Int(a + b),
            (LitValue::Int(a), "-", LitValue::Int(b)) => LitValue::Int(a - b),
            (LitValue::Int(a), "*", LitValue::Int(b)) => LitValue::Int(a * b),
            ...
            (LitValue::Int(a), "%", LitValue::Int(b)) => LitValue::Int(a % b),
            (LitValue::Int(a), ">", LitValue::Int(b)) => LitValue::Bool(a > b),
            (LitValue::Int(a), "<", LitValue::Int(b)) => LitValue::Bool(a < b),
            (LitValue::Int(a), "==", LitValue::Int(b)) => LitValue::Bool(a == b),
            (LitValue::Int(a), "!=", LitValue::Int(b)) => LitValue::Bool(a != b),
            (LitValue::Int(a), "<=", LitValue::Int(b)) => LitValue::Bool(a <= b),
